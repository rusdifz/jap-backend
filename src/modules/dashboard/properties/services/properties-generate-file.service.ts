  ...
  async generatePDFDetailProperty(property_slug: string): Promise<Buffer> {
    return new Promise(async (resolve) => {
        ...
        for (const unit of getData.units) {
          dynamicRows.push({
            content: [
              unit.floor,
              unit.size.toString(),
              'Rp.  ' + getData.price_rent_sqm.toString(),
              'Rp.  ' + getData.service_charge.toString(),
              //   'Rp.  ' + unit.rent_sqm === 'null' || null
              //     ? '0'
              //     : unit.rent_sqm.toString(),
              'Rp. 0',
              unit.condition,
            ],
          });
      ...
      doc
        ...
        .row([
          'Security Deposit',
          { value: '3 month Rental + Service Charge', fontSize: 6 },
          'Yes',
          'Air Conditioning',
      ...
      doc
        ...
        .row(['tba', '3-6 Zone/Floor', 'separately', '100 va / sqm']);
      ...
      //table pertama berisi unit
      //table kedua berisi overtime dan deposit
      //table ketiga parking charge, minimum lease term
      //table keempat build spec
    });
  }
}
